  ...
  if (param->getNucleonPositionsFromFile() == 0) {
    ...
    if (A1 == 1) {
      ...
      if (param->getlightNucleusOption() != 1) {
        ...
        } else if (param->getlightNucleusOption() == 4) {
          // PGCM
          fileName = "O16_PGCM.in";
        }
        ...
        if (param->getlightNucleusOption() == 4) {
          nucleusNumber = static_cast<int>(ran2 * 20000);
        }
        ...
        if (fin) {
          ...
          // start reading one nucleus (3 positions)
          int A = 0;
          if (param->getlightNucleusOption() == 2 || param->getlightNucleusOption() == 3) {
              fin >> dummy; // first two entries per nucleus are not coordinates
              fin >> dummy;
          }
          ...
          if (param->getlightNucleusOption() == 2)
            ...
            cerr << " file O16_PGCM.in not found. exiting." << endl;
          exit(1);
        }
        fin.close();
      } else {
      ...
      if (param->getlightNucleusOption() != 1) {
        ...
        // uniformly (6000 configurations in file)
        double ran2 =random->genrand64_real3();
        int nucleusNumber = static_cast<int>(ran2 * 20000);
        cout << "using nucleus Number = " << nucleusNumber << endl;
        ...
        if (fin) {
          ...
          if (param->getlightNucleusOption() == 2)
            cerr << " file Ne20_PGCM.in not found. exiting." << endl;
          exit(1);
        }
        fin.close();
      } else {
      ...
      if (param->getlightNucleusOption() != 1) {
        ...
        // sample the position in the file
        // uniformly (20000 configurations in file)
        double ran2 =random->genrand64_real3();
        int nucleusNumber = static_cast<int>(ran2 * 20000);
        cout << "using nucleus Number = " << nucleusNumber << endl;
        ...
        if (fin) {
          ...
          if (param->getlightNucleusOption() == 2)
            cerr << " file Ar40_plaintext.in not found. exiting." << endl;
          exit(1);
        }
        fin.close();
      } else {
    ...
    cout << "Reading nucleon positions from files 'nucleus1.dat' and "
            "'nucleus2.dat' ... "
         << endl;
    int A = 0;
    int A2 = 0;
...
void Init::setColorChargeDensity(Lattice *lat, Parameters *param,
  ...
  if (param->getUseNucleus() == 0) {
    ...
    if (!param->runTwoStage()) param->setSuccess(1);
    ...
    if (param->getGaussianWounding() == 0) {
      ...
      for (int i = 0; i < A1; i++) {
        for (int j = 0; j < A2; j++) {
          ...
          if (param->runTwoStage()) ran = ran_record_[ireco];
          ireco = ireco + 1;
...
// Get the shift impact b for the second stage IP-Glasma
void Init::Get_shift_impact_b(Lattice *lat, Parameters *param,
                                 Random *random, Glauber *glauber) {
  std::cout << "Get the shift impact b ..." << std::endl;
  int pos, posA, posB;
  int N = param->getSize();
  int A1, A2;
  // int check=0;
...
#pragma omp parallel for
  ...
  if (A1 < 4 && A2 > 1) {
    ...
    for (int i = 0; i < A1;
         i++) // shift the nuclei's position by -b/2 or +b/2 respectively
    {
      nucleusA_.at(i).x = nucleusA_.at(i).x - b / 2.; // shift by b here Wenbin
    }

    for (int i = 0; i < A2;
         i++) // shift the nuclei's position by -b/2 or +b/2 respectively
    {
      nucleusB_.at(i).x = nucleusB_.at(i).x + b / 2.; // shift by b here Wenbin
    }
  }
...
#pragma omp for
  ...
  if (param->getAverageQs() > 0 && param->getAverageQsAvg() > 0 &&
      averageQs2 > 0 && param->getAverageQsmin() > 0 && averageQs2Avg > 0 &&
      alphas > 0 && Npart >= 2 && averageQs2min2 * a * a / hbarc / hbarc > param->getMinimumQs2ST()) // criterion to select effective events//wenbin
    {
      param->setSuccess(1);
      stringstream strup_name;
      strup_name << "usedParameters_only_impact_b.dat";
      string up_name;
      up_name = strup_name.str();
...
void Init::init(Lattice *lat, Group *group, Parameters *param, Random *random,
  ...
  if (READFROMFILE > 0) {
    ...
    // Get the first stage shifted impact parameter
    if (param->runTwoStage()) {
        // record the shifted impact prameter at the first stage
        Get_shift_impact_b(lat, param, random, glauber);
    } else {
