...
void Evolution::evolveU(Lattice *lat, BufferLattice *bufferlat,
                        Parameters *param, double dtau, double tau) {
...
#pragma omp for
    for (int pos = 0; pos < N * N; pos++) {
      // retrieve current E1 and E2 (that's the one defined at half a time step
      // in the future (from tau))
...
void Evolution::evolvePhi(Lattice *lat, BufferLattice *bufferlat,
                          Parameters *param, double dtau, double tau) {
...
void Evolution::evolvePi(Lattice *lat, BufferLattice *bufferlat,
                         Parameters *param, double dtau, double tau) {
  const int Nc = param->getNc();
  const int N = param->getSize();
...
#pragma omp for
    for (int pos = 0; pos < N * N; pos++) {
      ...
      // phi_{-x} should be defined as UxD*phimX*Ux with the Ux and UxD reversed
      // from the phi_{+x} case retrieve current phi (at time tau) at x_T-1
      // parallel transport:
      UxXm1 = lat->cells[lat->posmX[pos]]->getUx();
      UyYm1 = lat->cells[lat->posmY[pos]]->getUy();

      phimX =
          Ux.prodAconjB(UxXm1, lat->cells[lat->posmX[pos]]->getphi()) * UxXm1;
      phimY =
          Ux.prodAconjB(UyYm1, lat->cells[lat->posmY[pos]]->getphi()) * UyYm1;

      bracket = phiX + phimX + phiY + phimY -
                4. * phi; // sum over both directions is included here

      pi += dtau /
            (tau)*bracket; // divide by \tau because this is computing
                           // pi(tau+dtau/2) from pi(tau-dtau/2) and phi(tau)
...
void Evolution::evolveE(Lattice *lat, BufferLattice *bufferlat,
                        Parameters *param, double dtau, double tau) {
  const int Nc = param->getNc();
  const int N = param->getSize();
  const double g = param->getg();
...
void Evolution::run(Lattice *lat, BufferLattice *bufferlat, Group *group,
                    Parameters *param) {
  int Nc = param->getNc();
  int pos;
  int N = param->getSize();
  ...
  for (int it = 1; it <= itmax; it++) {
    ...
    if (it < itmax) {
      ...
      evolvePi(lat, bufferlat, param, dtau / 2.,
               (it)*dtau); // the last argument is the current time tau.
        ...
        for (int iy = 0; iy < N; iy++) {
          ...
          if (param->getRunningCoupling()) {
            ...
            if (param->getRunWithQs() == 2) {
              if (g2mu2A > g2mu2B)
                Qs = sqrt(g2mu2A * param->getQsmuRatio() *
                          param->getQsmuRatio() / a / a * hbarc * hbarc *
                          param->getg() * param->getg());
              else
                Qs = sqrt(g2mu2B * param->getQsmuRatio() *
                          param->getQsmuRatio() / a / a * hbarc * hbarc *
                          param->getg() * param->getg());
              ...
              Qs = sqrt((g2mu2A + g2mu2B) / 2. * param->getQsmuRatio() *
                        param->getQsmuRatio() / a / a * hbarc * hbarc *
                        param->getg() * param->getg());
            }
            ...
            alphas =
                4. * M_PI /
                (9. * log(pow(pow(muZero / 0.2, 2. / c) +
                                  pow(param->getRunWithThisFactorTimesQs() *
                                          Qs / 0.2,
                                      2. / c),
                              c)));
...
void Evolution::Tmunu(Lattice *lat, Parameters *param, int it) {
  ...
  Matrix UDypY(Nc);
  Matrix UDymX(Nc);
  Matrix Uplaq(Nc), UplaqD(Nc), Uplaq1(Nc), Uplaq1D(Nc), Uplaq2(Nc), Uplaq3(Nc),
      Uplaq4(Nc);
  Matrix E1(Nc);
  Matrix E2(Nc);
  Matrix E1m(Nc);
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < N; j++) {
      ...
      Uplaq =
          lat->cells[pos]->getUx() * (lat->cells[posX]->getUy() * (UDx * UDy));
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < N; j++) {
      ...
      lat->cells[pos]->setTtautau(
          (g * g / (it * dtau) / (it * dtau) *
               real((E1 * E1).trace() + (E1p * E1p).trace() +
                    (E2 * E2).trace() + (E2p * E2p).trace()) /
               2. // trans.
           + (((pi * pi).trace()).real() + ((piX * piX).trace()).real() +
              ((piY * piY).trace()).real() + ((piXY * piXY).trace()).real()) /
                 4.)); // long.
      lat->cells[pos]->setTxx(
          (g * g / (it * dtau) / (it * dtau) *
               real(-1. * (E1 * E1).trace() - (E1p * E1p).trace() +
                    (E2 * E2).trace() + (E2p * E2p).trace()) /
               2. // trans.
           + (((pi * pi).trace()).real() + ((piX * piX).trace()).real() +
              ((piY * piY).trace()).real() + ((piXY * piXY).trace()).real()) /
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < N; j++) {
          ...
          1. / (it * dtau) / (it * dtau) *
              (-2. / pow(g, 2.) * (Nc - (Uplaq.trace()).real()) +
               0.5 / (it * dtau) / (it * dtau) *
                   (+real(((phi - phiTildeX) * (phi - phiTildeX)).trace()) +
                    real(
                        ((phiY - phiTildeXY1) * (phiY - phiTildeXY1)).trace()) +
                    real(((phi - phiTildeY) * (phi - phiTildeY)).trace()) +
      ...
      if (lat->cells[pos]->getg2mu2A() < 1e-12 ||
        ...
        lat->cells[pos]->setEpsilon(lat->cells[pos]->getTtautau() * 1 /
                                    pow(a, 4.));
      ...
      lat->cells[pos]->setTetaeta(lat->cells[pos]->getTetaeta() * 1 /
                                  pow(a, 6.));
    }
  }
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < N; j++) {
      ...
      // note that the minus sign of the first terms in T^\taux and T^\tauy
      // comes from the direction of the plaquettes - I am using +F^{yx} instead
      // of -F^{xy} if you like.
      lat->cells[pos]->setTtaux(
          +2. / (it * dtau) / 8. *
              (E2 * (Uy * UxpY * UDypX * UDx - Ux * UypX * UDxpY * UDy -
      ...
      lat->cells[pos]->setTtauy(
          +2. / (it * dtau) / 8. *
              (E1 * (Ux * UypX * UDxpY * UDy - Uy * UxpY * UDypX * UDx -
      ...
      lat->cells[pos]->setTtaueta(
          ...
          (E1 * (Ux * phiX * UDx - phi) + E1p * (UxpY * phiXY * UDxpY - phiY) +
           E2 * (Uy * phiY * UDy - phi) + E2p * (UypX * phiXY * UDypX - phiX))
              .trace()
              .real());
           ...
           1. / 4. *
               ((Ux * phiX * UDx - phi) * (Uy * phiY * UDy - phi) +
                Uy * (UxpY * phiXY * UDxpY - phiY) * UDy *
                    (Uy * phiY * UDy - phi) +
                (Ux * phiX * UDx - phi) * Ux * (UypX * phiXY * UDypX - phiX) *
                    UDx +
      ...
      lat->cells[pos]->setTxeta(
          -2. / (it * dtau) / (it * dtau) *
          (1. / 4. * g *
               (E1 * (pi + Ux * piX * UDx) + E1p * (piY + UxpY * piXY * UDxpY))
                   .trace()
                   .real() +
           1. / 8. / g *
               ((Ux * UypX * UDxpY * UDy - Uy * UxpY * UDypX * UDx -
                 (Ux * UypX * UDxpY * UDy - Uy * UxpY * UDypX * UDx).trace() /
                     static_cast<double>(Nc) * one +
                ...
                (UypX * UDxpY * UDy * Ux - UDx * Uy * UxpY * UDypX -
                 (UypX * UDxpY * UDy * Ux - UDx * Uy * UxpY * UDypX).trace() /
                     static_cast<double>(Nc) * one +
                 UxpX * Uyp2X * UDxpXpY * UDypX -
                 UypX * UxpXpY * UDyp2X * UDxpX -
                 (UxpX * Uyp2X * UDxpXpY * UDypX -
      ...
      lat->cells[pos]->setTyeta(
          -2. / (it * dtau) / (it * dtau) *
          (1. / 4. * g *
               (E2 * (pi + Uy * piY * UDy) + E2p * (piX + UypX * piXY * UDypX))
                   .trace()
                   .real() +
           1. / 8. / g *
               ((Uy * UxpY * UDypX * UDx - Ux * UypX * UDxpY * UDy -
                 (Uy * UxpY * UDypX * UDx - Ux * UypX * UDxpY * UDy).trace() /
                     static_cast<double>(Nc) * one +
                ...
                (UxpY * UDypX * UDx * Uy - UDy * Ux * UypX * UDxpY -
                 (UxpY * UDypX * UDx * Uy - UDy * Ux * UypX * UDxpY).trace() /
                     static_cast<double>(Nc) * one +
                 UypY * Uxp2Y * UDypXpY * UDxpY -
                 UxpY * UypXpY * UDxp2Y * UDypY -
                 (UypY * Uxp2Y * UDypXpY * UDxpY -
      ...
      lat->cells[pos]->setTtaueta(lat->cells[pos]->getTtaueta() * 1 /
                                  pow(a, 5.));
...
void Evolution::eccentricity(Lattice *lat, Parameters *param, int it,
                             double cutoff, int doAniso) {
  ...
  for (int ix = 0; ix < N; ix++) {
    ...
    for (int iy = 0; iy < N; iy++) {
      ...
      if (param->getRunningCoupling()) {
        ...
        if (param->getRunWithQs() == 2) {
          if (g2mu2A > g2mu2B)
            Qs = sqrt(g2mu2A * param->getQsmuRatio() * param->getQsmuRatio() /
                      a / a * hbarc * hbarc * param->getg() * param->getg());
          else
            Qs = sqrt(g2mu2B * param->getQsmuRatio() * param->getQsmuRatio() /
                      a / a * hbarc * hbarc * param->getg() * param->getg());
        } else if (param->getRunWithQs() == 0) {
          if (g2mu2A < g2mu2B)
            Qs = sqrt(g2mu2A * param->getQsmuRatio() * param->getQsmuRatio() /
        ...
        if (param->getRunWithLocalQs() == 1) {
          ...
          alphas =
              4. * M_PI /
              (9. *
               log(pow(pow(muZero / 0.2, 2. / c) +
                           pow(param->getRunWithThisFactorTimesQs() * Qs / 0.2,
                               2. / c),
                       c)));
          gfactor = g * g / (4. * M_PI * alphas);
          // run with the local (in transverse plane) coupling
      ...
      if (lat->cells[pos]->getEpsilon() * gfactor <
          cutoff) // this is 1/fm^4, so Lambda_QCD^{-4} (because \Lambda_QCD is
                  // roughly 1/fm)
      {
        weight = 0.;
      } else {
        ...
        g2mu2A = lat->cells[pos]->getg2mu2A();
        g2mu2B = lat->cells[pos]->getg2mu2B();
        avgQs2AQs2B += g2mu2A * param->getQsmuRatio() * param->getQsmuRatio() *
                       g2mu2B * param->getQsmuRatioB() *
                       param->getQsmuRatioB() / a / a / a / a;
      }
      avx += x * weight;
      avy += y * weight;
      toteps += weight;
  ...
  if (doAniso == 0) {
    ...
    foutEcc << it * a * param->getdtau() << " " << eccentricity1 << " " << Psi1
            << " " << eccentricity2 << " " << Psi2 << " " << eccentricity3
            << " " << Psi3 << " " << eccentricity4 << " " << Psi4 << " "
            << eccentricity5 << " " << Psi5 << " " << eccentricity6 << " "
            << Psi6 << " " << cutoff << " " << sqrt(avrSq) << " " << maxX << " "
            << maxY << " " << param->getb() << " " << param->getTpp() << " "
  ...
  if (doAniso == 1) {
    ...
    for (int ix = 0; ix < N; ix++) {
      for (int iy = 0; iy < N; iy++) {
        ...
        TxxRot = cos(Psi) * (cos(Psi) * lat->cells[pos]->getTxx() -
                             sin(Psi) * lat->cells[pos]->getTxy()) -
                 sin(Psi) * (cos(Psi) * lat->cells[pos]->getTxy() -
                             sin(Psi) * lat->cells[pos]->getTyy());
        TyyRot = sin(Psi) * (sin(Psi) * lat->cells[pos]->getTxx() +
                             cos(Psi) * lat->cells[pos]->getTxy()) +
                 cos(Psi) * (sin(Psi) * lat->cells[pos]->getTxy() +
...
void Evolution::readNkt(Parameters *param) {
  ...
  for (int ik = 0; ik < 100; ik++) {
    if (param->getUsePseudoRapidity() == 0) {
      ...
      dNdeta2 += nIn[ik] * (ik + 0.5) * dkt * dkt * 2. * M_PI *
                 ...
                 (sqrt(pow(cosh(param->getRapidity()), 2.) +
  ...
  foutNN << Npart << " " << dNdeta << " " << dNdeta2 << " " << atof(Tpp.c_str())
         << " " << atof(b.c_str()) << endl;
  foutNN.close();
...
int Evolution::multiplicity(Lattice *lat, Group *group, Parameters *param,
                            int it) {
  int N = param->getSize();
  int Nc = param->getNc();
  int npos, pos;
    ...
    for (int j = 0; j < N; j++) {
      ...
      if (param->getRunWithkt() == 0) {
        *E1[pos] = lat->cells[pos]->getE1() *
                   sqrt(gfactor); // replace one of the 1/g in the lattice E^i
                                  // by the running one
      } else {
        *E1[pos] = lat->cells[pos]->getE1();
      }
    ...
    for (int j = 0; j < N; j++) {
      ...
      omega2 =
          ...
          (sin(kx / 2.) * sin(kx / 2.) +
           sin(ky / 2.) *
               sin(ky /
                   2.)); // lattice dispersion relation (this is omega squared)
      if (i != 0 && j != 0) {
        if (omega2 != 0) {
        ...
        for (int ik = 0; ik < bins; ik++) {
          if (abs(sqrt(kt2)) > ik * dkt && abs(sqrt(kt2)) <= (ik + 1) * dkt) {
            n[ik] += nkt / dkt / 2 / M_PI / sqrt(kt2) * 2 * M_PI * sqrt(kt2) *
                     dkt * N * N / M_PI / M_PI / 2. / 2.;
            E[ik] += sqrt(omega2) * hbarc / a * nkt / dkt / 2 / M_PI /
                     sqrt(kt2) * 2 * M_PI * sqrt(kt2) * dkt * N * N / M_PI /
                     M_PI / 2. / 2.;
            n2[ik] += nkt / dkt / 2 / M_PI / sqrt(kt2);
            // dividing by bin size; bin is dkt times Jacobian k(=ik*dkt) times
            // 2Pi in phi times the correct number of counts for an infinite
            // lattice: area in bin divided by total area
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < N; j++) {
      ...
      if (param->getRunWithkt() == 0) {
        *E1[pos] =
            lat->cells[pos]->getpi() *
            sqrt(gfactor); // replace the only 1/g by the running one (physical
                           // pi goes like 1/g, like physical E^i)
      } else {
        ...
        dNdeta2 += n[ik] * (ik + 0.5) * dkt * dkt * 2. * M_PI *
                   ...
                   (sqrt(pow(cosh(param->getRapidity()), 2.) +
                         m * m /
                             (((ik + 0.5) * dkt / a * hbarc) *
                              ((ik + 0.5) * dkt / a *
                               hbarc)))); // integrate, gives a ik*dkt*2pi*dkt
  ...
  if (it == itmax && param->getWriteOutputs() == 3) {
    ...
    // dEdetaHadronsCut = 2*M_PI*gsl_spline_eval_integ(ptspline, 3., 19.,
    // ptacc); dEdetaHadronsCut2 =
    // 2*M_PI*gsl_spline_eval_integ(ptspline, 6., 19., ptacc);
...
int Evolution::multiplicitynkxky(Lattice *lat, Group *group, Parameters *param,
                                 int it) {
  const int N = param->getSize();
  const int Nc = param->getNc();
  int npos, pos;
  ...
  for (int ik = 0; ik < bins; ik++) {
    ...
    if (it > 0) {
      foutMult << it * dtau * a << " " << ik * dkt / a * hbarc << " "
               ...
               << " " << param->getb() << " " << param->getNpart() << endl;
    }
  }
  ...
  if (it == itmax) {
    ...
    foutNNH << param->getNpart() << " " << dNdetaHadrons << " "
            << param->getTpp() << " " << param->getb() << " " << dEdetaHadrons
            << " " << param->getRandomSeed() << " "
            << "N/A"
            << " "
            << "N/A"
...
int Evolution::correlations(Lattice *lat, Group *group, Parameters *param,
                            int it) {
  const int N = param->getSize();
  const int Nc = param->getNc();
  int npos, pos;
  ...
  for (int i = 0; i < N; i++) {
    for (int j = 0; j < N; j++) {
      ...
      if (i != 0 && j != 0) {
        if (omega2 != 0) {
          nkt1 = 1. / sqrt(omega2) / static_cast<double>(N * N) *
                 (1. / ((it - 0.5) * dtau) *
                  ((((*E1[pos]) * (*E1[npos])).trace()).real() +
          ...
          nkt3 = sqrt(omega2) / static_cast<double>(N * N) *
                 ((it)*dtau * ((((*A1[pos]) * (*A1[npos])).trace()).real() +
                               (((*A2[pos]) * (*A2[npos])).trace()).real()));

          nkt4 = sqrt(omega2) / static_cast<double>(N * N) *
                 (1. / ((it)*dtau) *
                  ((((*phi[pos]) * (*phi[npos])).trace()).real()));

          nkt5 = 1. / static_cast<double>(N * N) *
                 (complex<double>(0., 1.) *
                  ((*E1[pos]) * (*A1[npos]) - (*A1[pos]) * (*E1[npos]) +
                   (*E2[pos]) * (*A2[npos]) - (*A2[pos]) * (*E2[npos]))
                      .trace())
                     .real();

          nkt6 =
              1. / static_cast<double>(N * N) *
              (complex<double>(0., 1.) *
               ((*pi[pos]) * (*phi[npos]) - (*phi[pos]) * (*pi[npos])).trace())
                  .real();
  ...
  for (int ik = 1; ik < bins; ik += 4) {
    for (int iphi = 0; iphi < phiBins; iphi++) {
      foutPhiMult
          ...
          << endl; //<< " " << nNoMixedTerms[ik][iphi]*a/hbarc*a/hbarc << endl;
    }
  }
  foutPhiMult.close();
  ...
  for (int iphi = 0; iphi < phiBins; iphi++) {
    for (int ih = 0; ih <= hbins; ih++) {
      mypt = ih * dkt / a *
             hbarc; //(10./static_cast<double>(hbins));  // the hadron's p_T
      ...
      for (int iz = 0; iz < steps; iz++) {
        ...
        if (ik + 1 < bins && ik >= 0) {
          Ng = ((1. - frac) * n[ik][iphi] + frac * n[ik + 1][iphi]) * a /
               hbarc * a / hbarc; // to make dN/d^2k_T fo k_T in GeV
          if (kt > 2)
            Ng *= exp(-(kt - 2) * 0.5);
        } else
          Ng = 0.;
